...
const displayCharacters = async (name) => {
    ...
    if (characters[0] != undefined && characters.length > 0) {
        if (!target.classList.contains("cardPool")) {
        ...
        characters.forEach((character) => {
            ...
            clone.querySelector(".card__p").innerHTML =
        ...
        clone.querySelector(
            ".card__img"
        ).src = `data:image/png;base64,${characters.image}`;
        clone.querySelector(".card__h3").innerHTML = characters.name;
        clone.querySelectorAll(".card__p")[0].innerHTML =
            characters.shortDescription;
        clone.querySelectorAll(".card__p")[1].innerHTML =
        ...
        if (document.getElementsByClassName("card__button--addnew")[0]) {
            document.body.removeChild(
                document.getElementsByClassName("card__button--addnew")[0]
            );
        }
        let btnSee =
            clone.children[0].getElementsByClassName("card__button")[0];
